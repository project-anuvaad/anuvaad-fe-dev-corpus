        ...
        for (let row in sentences) {
            ...
            let blockData = this.props.paperType === 'source' ? sentences[row][block].text : sentences[row][block].target
                ...
                col.push(<td id={sentences[row][block].node_index}
                    ...
                    style={{ backgroundColor: (this.props.hoveredTableId === sentences[row][block].node_index) ? "yellow" : '', padding: '8px', border: '1px solid black', borderCollapse: 'collapse' }}>
                    {blockData}</td>)
            }
    ...
    fetchSentence(sentence) {
        ...
        if (!sentence.is_footer && sentence.text) {
            if (sentence.is_ner && !sentence.is_new_line) {
                return (<div key={sentence._id} onClick={() => this.props.handleSentenceClick(sentence._id)} onMouseEnter={() => this.hoverOn(sentence._id)} 
                    style={{
                        backgroundColor: (this.props.hoveredSentence === sentence._id) ? "yellow" : '', float: align, textAlign: align, display: 'inline-block',
                ...
                return (<div><div key={sentence._id} onClick={() => this.props.handleSentenceClick(sentence._id)} onMouseEnter={() => this.hoverOn(sentence._id)} 
                    style={{
                        backgroundColor: (this.props.hoveredSentence === sentence._id) ? "yellow" : '', float: align, textAlign: align,
                        fontWeight: sentence.is_bold ? 'bold' : 'normal', textDecorationLine: sentence.underline ? 'underline' : ''
                ...
                return (<div key={sentence._id} onClick={() => this.props.handleSentenceClick(sentence._id)} onMouseEnter={() => this.hoverOn(sentence._id)} 
                    style={{
                        backgroundColor: this.props.hoveredSentence === sentence._id ? "yellow" : '', textAlign: align, right: 0,
                        fontWeight: sentence.is_bold ? 'bold' : 'normal', textDecorationLine: sentence.underline ? 'underline' : ''
